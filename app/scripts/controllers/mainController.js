...
angular.module('ethExplorer')
    .controller('mainCtrl', function ($rootScope, $scope, $location, $http, NgTableParams) {
        ...
        function updateStats() {
		  ...
		  $scope.blockNumber = $scope.blockNum;
      ...
      // }
		...
		function updateTXList(){
			...
			//function ($scope, $http){
				...
				$http.get('http://167.114.100.39:5099/txhash').then(function(tranphp){
							
							//$scope.addresstransactions = tranphp.data.records;
							console.log('tranphp '+tranphp);
							$scope.recenttransactions = tranphp.data;	
							//var self = this;
							$scope.tableParams = new NgTableParams({}, {dataset: $scope.recenttransactions});
				});
			//});
		}
        ...
        // function updateTXList() {
			...
			// console.log('updating tx list: '+ $scope.blockNum);
			// var currentTXnumber = $scope.blockNum; 
            // $scope.txNumber = currentTXnumber;
            // $scope.recenttransactions = [];
			// var maxnum = 10;
			// var counttran = 0;
			
            // loop1: for (var i=0; i < 100 && currentTXnumber - i >= 0; i++) {
				// var count = web3.eth.getBlockTransactionCount(currentTXnumber - i);
				// console.log('count ..'+ count);
				
				// loop2: for(var y =0; y < count; y++){
					
					// var tran = web3.eth.getTransactionFromBlock(currentTXnumber - i,y);
					// //console.log(' tran: '+tran);
					// var inputData;
				  // if(tran != null && tran.input != '0x'){
					 // inputData = tran.input;
					 // //console.log('inputData '+ inputData);
					 					 	 
					 // tran.to = '0x' + inputData.substr(34,40);
					 // //console.log('tran.to 0x'+ inputData.substr(34,40));
					
					 // var trcval = inputData.substr(106, 32);
					 // tran.value = parseInt(trcval, 16) / 100000000;
					 // //console.log('tran.value ' + trcval);
					 // counttran++;
					 // $scope.recenttransactions.push(tran);
					 // if(counttran > 5){
        ...
        function updateBlockList(blockNum) {
            //var currentBlockNumber = web3.eth.blockNumber;
			var currentBlockNumber = blockNum;
            $scope.blockNumber = currentBlockNumber;
            $scope.blocks = [];
            for (var i=0; i < 5 && currentBlockNumber - i >= 0; i++) {
